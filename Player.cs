...
public class Player : MonoBehaviour
{
    ...
    public Transform claymore3;
    public Transform claymore4;
    ...
    bool isInteractingWithQuestNPC = false;
    ...
    private void Update()
    {
        ...
        {
            print("asdasd");
            if (DataManager.instance.GetSaveData().Skin)
            {
                int index = DataManager.instance.GetSaveData().EquipSkin;
                if (++index >= playerskin.Length)
                    index = 0;

                SkinChange(index); // 스킨 변경 메서드 호출
            }
        }
    }
    ...
    void EndClimbingUp()
        ...
        transform.position = anim.gameObject.transform.position;
        ...
        swaping = true;
            ...
            claymore3.gameObject.SetActive(nomallMode);
            claymore4.gameObject.SetActive(fightMode);
        swaping = false;
        }
    ...
    void InteractWithQuestNPC()
    {
        if (isInteractingWithQuestNPC)
        {
            if (eDown)
            {
                if (DataManager.instance.GetSaveData().Skin)
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        {
            isInteractingWithQuestNPC = true;
        }
    ...
    void OnTriggerExit(Collider other)
    {
        ...
        if (other.CompareTag("QuestNPC"))
        {
            isInteractingWithQuestNPC = false;
            speechBubble2.SetActive(false);
    ...
    public void CheckPlayerActive()
    {
        ...
        GameObject player = playerskin[DataManager.instance.GetSaveData().EquipSkin];
        player.SetActive(true);
        SetAnim(player.GetComponent<Animator>());
    }

    public void SkinChange(int index)
    {
        DataManager.instance.GetSaveData().EquipSkin = index;
